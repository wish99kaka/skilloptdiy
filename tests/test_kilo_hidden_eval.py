...
MODULE_PATH = ROOT / "work/run_kilo_hidden_eval.py"


def load_eval_module():
    spec = importlib.util.spec_from_file_location("run_kilo_hidden_eval", MODULE_PATH)
...
class KiloHiddenEvalTests(unittest.TestCase):
    def test_rewrites_agent_command_and_preserves_task_dir(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch.dict("os.environ", {"KILO_TASK_TIMEOUT": "777"}):
                rewritten = module.build_kilo_tasks(task_file, Path("/tmp/kilo_agent_wrapper.py"))

            payload = json.loads(rewritten.read_text(encoding="utf-8"))

        metadata = payload["metadata"]
        self.assertIn("kilo_agent_wrapper.py", metadata["agent_command"])
    ...
    def test_can_limit_task_count(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch.dict("os.environ", {"KILO_TASK_LIMIT": "1"}):
                rewritten = module.build_kilo_tasks(task_file, Path("/tmp/kilo_agent_wrapper.py"))
